...
# Expect drivers shipped alongside installed app (resources/drivers/)
$interceptionDir = Join-Path $root "drivers\interception"
$vjoyDir = Join-Path $root "drivers\vjoy"
...
if (-not (Test-Path $interception)) {
  if ($Download) {
    ...
    Write-Info "Use -Download flag to download from internet, or ensure drivers are bundled with the app"
  }
}
