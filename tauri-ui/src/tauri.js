// tauri-ui/src/ui.js
// UI logic extracted from the old inline <script>.
// This file is meant to be imported by src/main.js and its functions exposed on `window`
// so your HTML inline onclick="..." continues to work.
...
export let mhSelected = null; // { gameId, model, kind, _el? }
...
export function toast(title, msg, kind = "ok") {
  ...
  el.innerHTML = `<div class="t-title">${escapeHtml(title)}</div><div class="t-msg">${escapeHtml(
    msg
  )}</div>`;
  wrap.appendChild(el);
...
export function logSystem(msg) {
  ...
  term.innerHTML += `<div class="log-entry"><span class="log-time">[${escapeHtml(
    time
  )}]</span> ${escapeHtml(msg)}</div>`;
  term.scrollTop = term.scrollHeight;
}

// This is called by src/main.js (listen("terminal_update"...))
export function update_terminal(line) {
  // accept non-string payloads too
  const text = typeof line === "string" ? line : JSON.stringify(line);
  logSystem(text);

  // progress parsing: "Epoch X/Y"
...
// -------------------------
// AI Calls (Gemini + OpenAI via fetch)
// NOTE: network calls are allowed by CSP connect-src
// -------------------------
...
export async function toggleRecord(btn) {
  ...
  if (isRecording) {
    try {
      await api.startRecording(activeGameId, dsName);
    } catch (e) {
      console.error(e);
      isRecording = false;
      toast("Recording", "Failed to start recording.", "err");
      return;
    }
    ...
    if (status) {
      status.innerText = "🔴 Recording... Stop to archive dataset into catalog.";
    ...
    if (status) {
      status.innerText = "✅ Stopped. If new data was detected, it was archived into datasets/.";
...
export async function toggleBot(btn) {
  ...
  if (isBotRunning) {
    try {
      ...
      console.error(e);
      isBotRunning = false;
      toast("Bot", "Failed to start bot.", "err");
      return;
    }
...
export async function mhInit() {
  ...
  try {
    ...
    if (!ok) {
      if (status) status.innerText = "ModelHub missing";
      toast("ModelHub", "Python bridge missing: scripts/modelhub_cli.py", "err");
      return;
    }
  } catch (e) {
    console.error(e);
    if (status) status.innerText = "ModelHub error";
    toast("ModelHub", "Backend invoke failed.", "err");
    return;
  }
  ...
  let gamesPayload;
  try {
    gamesPayload = await api.modelhubListGames();
  } catch (e) {
    console.error(e);
    toast("ModelHub", "Failed to list games.", "err");
    return;
  }

  // gamesPayload might be array or {ok:true, games:[...]}
  const games = Array.isArray(gamesPayload) ? gamesPayload : gamesPayload?.games || gamesPayload?.data || [];
...
export async function refreshCatalog() {
  ...
  setDisabled("mh-validate", true);
  setDisabled("mh-eval", true);
  setDisabled("mh-set-active", true);
  setDisabled("mh-run", true);
  ...
  try {
    ...
    if (builtins.length > 0 && list) {
      ...
      sep.innerHTML = `<span class="log-time">[Built-in]</span> Testing models (ship with the app)`;
      list.appendChild(sep);

      for (const m of builtins) {
        list.appendChild(renderModelItem(gid, m, "builtin", activeId));
      }
    }
    ...
    if (list) {
      ...
      sep2.className = "log-entry";
      sep2.style.padding = "10px";
      sep2.style.color = "#d1d5db";
      sep2.innerHTML = `<span class="log-time">[Trained]</span> Archived models (trained_models/)`;
      list.appendChild(sep2);

      if (managed.length === 0) {
        const none = document.createElement("div");
        none.className = "log-entry";
        none.style.padding = "10px";
        none.style.color = "var(--text-dim)";
        none.innerHTML = `<span class="log-time">[None]</span> No trained models yet. Use Train Wizard.`;
        list.appendChild(none);
      } else {
        for (const m of managed) {
          list.appendChild(renderModelItem(gid, m, "managed", activeId));
        }
      }
    }

    // update wizard datasets too
    await twRefreshDatasets(true);
    toast("Catalog", "Updated catalog.", "ok");
  } catch (e) {
    console.error(e);
    if (list) list.innerHTML = `<div class="log-entry" style="padding:10px;"><span class="log-time">[Error]</span> Failed to load catalog.</div>`;
    toast("Catalog Error", "Failed to load catalog from backend.", "err");
  }
}
...
function renderModelItem(gameId, model, kind, activeId) {
  ...
  const created = model.created_at || model.profile?.created_at || "";
  ...
  item.innerHTML = `
    <div class="top">
      <div>
        ...
        <div class="m-sub">${escapeHtml(id)} ${created ? "• " + escapeHtml(created) : ""}</div>
      </div>
    ...
    <div class="m-tags">
      <span class="tag">🎮 ${escapeHtml(gameId)}</span>
      <span class="tag">🧬 ${escapeHtml(arch)}</span>
      ${path ? `<span class="tag">📁 ${escapeHtml(path)}</span>` : ``}
    </div>
  `;
...
export function mhSelectModel(gameId, model, kind, el) {
  ...
  setDisabled("mh-validate", false);
  setDisabled("mh-eval", false);
  setDisabled("mh-set-active", false);
  setDisabled("mh-run", false);
}
...
export async function mhValidate() {
  ...
  const modelDir =
    kind === "builtin"
      ? model.paths?.dir || model.dir || model.path || ""
      : model.path || model.paths?.dir || model.dir || "";

  if (!modelDir) {
    toast("Validate", "No model directory available.", "err");
    return;
  }

  const res = await api.mhValidate(gameId, modelDir);
  if (res?.ok) toast("Validate", "Model profile OK.", "ok");
  else toast("Validate", res?.message || "Invalid model.", "warn");

  logSystem(res?.ok ? `✅ Model valid: ${res?.message || ""}` : `⚠️ Model invalid: ${res?.message || ""}`);
}
...
export async function mhEval() {
  ...
  if (!ds) {
    alert("Please set a dataset folder.");
    return;
  }
  ...
  logSystem(res?.ok ? `▶ Offline evaluation started.` : `❌ Eval failed: ${res?.message || ""}`);
  toast("Evaluation", res?.ok ? "Started offline evaluation." : "Evaluation failed.", res?.ok ? "ok" : "err");
}
...
export async function mhSetActive() {
  ...
  const { gameId, model, kind } = mhSelected;
  const modelId = model.id || model.model_id || model.name || "unknown";
  const modelPath =
    kind === "builtin"
      ? model.paths?.dir || model.dir || model.path || ""
      : model.path || model.paths?.dir || model.dir || "";
  ...
  if (!modelPath) {
    toast("Active Model", "Missing path for model.", "err");
    return;
  }

  const res = await api.mhSetActive(gameId, modelId, modelPath);
  if (res?.ok === false) {
    toast("Active Model", res?.error || "Failed to set active model.", "err");
    return;
  }
  ...
  const runActive = document.getElementById("run-active-model");
  if (runActive) runActive.innerText = modelId;
...
export function openTrainWizard() {
  ...
  if (nameInput && !nameInput.value) {
    nameInput.value = "model_" + new Date().toISOString().slice(0, 10).replaceAll("-", "");
  }

  const g = activeGameId || DEFAULT_GAME;
  const twGame = document.getElementById("tw-game");
  if (twGame) twGame.value = g;

  twRefreshDatasets(true);
}

export function closeTrainWizard() {
  document.getElementById("train-wizard-overlay")?.classList.remove("open");
}

export function twBack() {
  if (twStep > 1) {
    twStep -= 1;
    twRender();
  }
...
export async function twNext() {
  if (twStep === 1) {
    ...
    if (!name) {
      toast("Train Wizard", "Please enter a model name.", "warn");
      return;
    }
    ...
    const mhGame = document.getElementById("mh-game");
    if (mhGame) mhGame.value = activeGameId;
    setActiveGamePills(activeGameId);

    twStep = 2;
    twRender();
    await twRefreshDatasets();
    return;
  }

  if (twStep === 2) {
    const did = document.getElementById("tw-dataset")?.value || "";
    if (!did) {
      toast("Train Wizard", "Pick a dataset (or record one).", "warn");
      return;
    }
    twStep = 3;
    twRender();
  ...
  const modelName = (document.getElementById("tw-model-name")?.value || "").trim() || "Untitled";
  ...
  logSystem(`🧠 Training requested: name="${modelName}", game="${gameId}", dataset="${datasetId}", arch="${arch}"`);
  toast("Training", `Started: ${modelName}`, "ok");

  try {
    await api.startTraining(gameId, modelName, datasetId, arch);
  } catch (e) {
    console.error(e);
    toast("Training", "Failed to start training.", "err");
  }
}
...
export async function twRefreshDatasets(silent = false) {
  ...
  try {
    ...
    if (ds.length === 0) {
      dsSel.innerHTML = `<option value="">(No datasets yet)</option>`;
      if (!silent) toast("Datasets", "No datasets found. Record in Teach tab.", "warn");
      return;
    }
    ...
    for (const d of ds) {
      ...
      if (!did) continue;
      const label = `${d.name || did}${d.created_at ? " — " + d.created_at : ""}`;
      dsSel.innerHTML += `<option value="${escapeHtmlAttr(did)}">${escapeHtml(label)}</option>`;
    }
  } catch (e) {
    console.error(e);
    if (!silent) toast("Datasets", "Failed to load datasets.", "err");
  }
}

function twRender() {
  const d1 = document.getElementById("tw-dot-1");
  const d2 = document.getElementById("tw-dot-2");
  const d3 = document.getElementById("tw-dot-3");
  ...
  if (l1) l1.style.width = "0%";
  if (l2) l2.style.width = "0%";

  if (twStep === 1) {
    d1?.classList.add("active");
  } else if (twStep === 2) {
    d1?.classList.add("done");
    if (l1) l1.style.width = "100%";
    d2?.classList.add("active");
  } else {
    d1?.classList.add("done");
    if (l1) l1.style.width = "100%";
    d2?.classList.add("done");
    if (l2) l2.style.width = "100%";
  ...
  const back = document.getElementById("tw-back");
  const next = document.getElementById("tw-next");
  if (back) back.disabled = twStep === 1;
  if (next) next.innerText = twStep === 3 ? "Start Training" : "Next";

  if (twStep === 3) {
    const n = (document.getElementById("tw-model-name")?.value || "").trim() || "Untitled";
    const g = document.getElementById("tw-game")?.value || DEFAULT_GAME;
    const did = document.getElementById("tw-dataset")?.value || "";
    const arch = document.getElementById("tw-arch")?.value || "custom";
    ...
    if (sum) {
      sum.innerHTML = `
        <div style="font-weight:900; color:#fff; margin-bottom:6px;">Summary</div>
        <div>Model: <b style="color:#fff;">${escapeHtml(n)}</b></div>
        <div>Game: <b style="color:#fff;">${escapeHtml(g)}</b></div>
        <div>Dataset: <b style="color:#fff;">${escapeHtml(did || "(none)")}</b></div>
        <div>Arch: <b style="color:#fff;">${escapeHtml(arch)}</b></div>
      `;
    }
  }
}
...
export async function sendChatMessage() {
  ...
  const formatted = escapeHtml(response)
    .replace(/\*\*(.*?)\*\*/g, "<b>$1</b>")
    .replace(/\n/g, "<br>");

  if (history) {
    history.innerHTML += `<div class="chat-bubble bubble-ai">${formatted}</div>`;
    history.scrollTop = history.scrollHeight;
  }
}
...
export async function initUI() {
  // Ensure provider dropdown updates input in modal
  const providerSel = document.getElementById("settings-provider");
  if (providerSel) providerSel.addEventListener("change", updateModalInput);

  // Load AI config from Rust
  try {
    const saved = await api.getAiConfig();
    if (saved) {
      aiConfig = saved;
      logSystem(`System Initialized. Active Provider: ${(aiConfig.provider || "gemini").toUpperCase()}`);
      const sel = document.getElementById("settings-provider");
      if (sel) sel.value = aiConfig.provider || "gemini";
      updateModalInput();
    }
  } catch (e) {
    console.error(e);
    toast("Config", "Failed to load config (invoke).", "err");
  }

  // Listen for terminal updates (Rust -> UI)
  try {
    await listen("terminal_update", (e) => update_terminal(e.payload));
  } catch (e) {
    console.error(e);
    toast("Events", "Failed to listen for terminal_update.", "err");
  }
